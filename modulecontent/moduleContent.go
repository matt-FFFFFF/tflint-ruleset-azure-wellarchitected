...
func FetchAttributes(f BlockFetcher, runner tflint.Runner) (*terraform.Evaluator, []*hclext.Attribute, hcl.Diagnostics) {
	...
	attrs, diags := getAttributes(ctx, config.Module, f)
...
func getAttributes(ctx *terraform.Evaluator, module *terraform.Module, bf BlockFetcher) ([]*hclext.Attribute, hcl.Diagnostics) {
	resources, diags := blocksWithPartialContent(ctx, module, bf)
	if diags.HasErrors() {
		return nil, diags
	}
